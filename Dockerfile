...
CMD ["sh", "-c", "python manage.py migrate --no-input && python manage.py ensure_admin && exec gunicorn config.wsgi:application --bind 0.0.0.0:${PORT:-8080} --workers 2 --timeout 60"]
